...
Given that the Airbnb dataset contains complex, non-linear interactions among numerical and categorical attributes (e.g., room type, number of amenities, and location rating), kNN provides a simple yet effective baseline for identifying patterns in multi-dimensional feature space.

### Data Preprocessing
Since kNN relies on distance-based similarity, all input features were converted to numerical and standardized to comparable scales.  
The categorical variable room_type was one-hot encoded using caret::dummyVars, ensuring that categorical differences are captured without imposing artificial order.  
To enhance stability and avoid redundancy in distance calculations, we performed numerical feature screening in three steps:  
...
All numeric variables were then centered and scaled to prevent large-valued features from dominating the Euclidean distance metric.  
...
## K Nearest Neighbour (kNN)
After tuning the neighbourhood size (k = 27), the final kNN model was tested on the 20% hold-out dataset.
The model reached an overall accuracy of 0.7079, showing that about 71% of listings were correctly classified into their price range.
The macro-weighted F1 score (0.7087) and MCC (0.5507) also indicate that the model performed consistently across the three price classes and achieved a balanced prediction between precision and recall.
```{r}
...
From the confusion matrix (Figure X), most listings in the middle price range [150,300) were correctly predicted, while some overlap appeared between neighbouring groups.
Low-price listings [0,150) were sometimes classified as mid-price, and a few high-price listings [300,+) were also predicted as mid-price.
This is reasonable because listings close to the price boundaries often share similar features, such as comparable room types or amenities.
Overall, the kNN model produced stable and reasonable results, effectively identifying listings with similar characteristics while keeping misclassification mainly within adjacent price categories.
As kNN is a distance-based, non-probabilistic model, probability-based metrics such as ROC-AUC and Brier score were not applicable in this evaluation.
